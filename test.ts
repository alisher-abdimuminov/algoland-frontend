...
const markdown = `
> :icon{name=lisp} Example headings
> Sample Section
> - This'll be a Helpful Section $$About the Greek Letter Θ$$!
> - :icon{name=erlang} A heading containing characters not allowed in fragments, UTF-8 characters, two consecutive spaces between the first and second words, and formatting.
> - This heading \`is not unique\` in the file
> TEXT 1
> This :icon{name=ts} heading is not unique in the file
> TEXT 2 :icon{name=cobol}
> Links to the example headings above
> Link to the :icon{name=d} sample section: Link Text.
> Link to the helpful section: Link Text.
> Link to the first non-unique section: Link Text.
> Link to the second non-unique section: Link Text :icon{name=python}
> :icon{name=cpp}
`;
...
function convertToVNode(node: Content): VNode {
    ...
    // Image
        ...
        return h(AspectRatio, {
            ...
            default: () => h("img", { src: node.url, alt: node.alt, class: "object-cover w-full h-full rounded-md" })
        });
    }
    ...
    // Math
        ...
        return h("code", { class: "cursor-pointer block inline-flex gap-1 items-center justify-center", innerHTML: katex.renderToString(node.value, { throwOnError: false }) });
    }
    ...
    // Leaf directives
        ...
        if (node.name === "icon") {
            return h(Icon, { name: node.attributes ? node.attributes.name : "" })
        }
    }
...
// 🔄 Barcha tugunlarni Vue formatiga o‘girish
export const nodes: VNode[] = tree.children.map(convertToVNode);
